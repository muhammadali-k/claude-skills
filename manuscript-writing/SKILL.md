...
6. **Results describe; interpretation lives only in the Discussion.** The Results section is a pure, descriptive transcription of the exhibits — report what was measured, in whom, and the value with its exhibit pointer, then stop. **No interpretation in Results**: no mechanism, no "this suggests / likely reflects / because / consistent with / indicating," no clinical significance, no comparison to prior work. All of that is quarantined to the Discussion. Write Methods as an ordered pipeline of short labeled subsections and title each Results subsection *identically*, so a variable tracks from method to result by name; Results always opens with the total N/denominator. (If a Results sentence would still be true with the numbers removed, it's interpretation — move it to the Discussion.)
7. **Template-and-swap parallelism, then tether to an exhibit.** When the same measure recurs across subgroups, arms, or subtypes, write the sentence **once and reuse it verbatim** for each stratum — change *only* the stratum name and its numbers; keep the sentence structure, verb, clause order, and punctuation identical (the PV/ET/MF order is recycled word-for-word across sentence, table, and figure, so strata line up column-wise). Report the overall population first, then the strata in the same fixed order every time. End every quantitative claim by pointing to a specifically numbered exhibit (*Table 2; Supplement Figure S11*); Results cite exhibits, never the bibliography, with detail pushed to a heavily indexed supplement.
