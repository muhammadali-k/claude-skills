...
5. Build the calendar file: `python3 scripts/openhouse.py ics --run-date $RUN_DATE` (writes `data/runs/$RUN_DATE.ics` with every upcoming event). Then email the digest with the Gmail connector to kmuhammadali0224@gmail.com. Subject: `IM open houses — $RUN_DATE: <N> new`. Body: the digest converted to simple HTML (headings, numbered list, links clickable) — never raw markdown `#` characters. **Attach the .ics file** (base64, mimeType `text/calendar`, filename `im-open-houses-$RUN_DATE.ics`) so he can add everything to his calendar in one tap. Send even when N = 0 (one line saying nothing new + what was checked).
6. Calendar: `python3 scripts/openhouse.py calendar-json --run-date $RUN_DATE` prints the events not yet on the calendar. Try to load the Google-Calendar create tool (ToolSearch `select:mcp__Google-Calendar__create_event`). If it exists, create each event on the primary calendar (`summary`, `description`, `start`/`end` with the event's own `timezone`, or `start_date`/`end_date` for all-day) and after each success run `python3 scripts/openhouse.py mark <id> announced --calendar-added` (use `registered` if that is its current status). If the tool does not exist (only `search_events` is exposed — observed 2026-09-01), skip this step, say so in the run note, and rely on the .ics attachment; leave the events queued.
