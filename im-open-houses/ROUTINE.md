...
| Connectors | Gmail (send digest), Google-Calendar (create events) |
...
6. Calendar: `python3 scripts/openhouse.py calendar-json --run-date $RUN_DATE` prints the events not yet on the calendar. Create each one on the user's primary Google Calendar with the Google-Calendar connector: use `summary`, `description`, and `start`/`end` with the event's own `timezone` (or `start_date`/`end_date` for all-day). After each successful creation run `python3 scripts/openhouse.py mark <id> announced --calendar-added` (if the event's current status is `registered`, use `registered` instead of `announced`).
7. `python3 scripts/openhouse.py commit-run --run-date $RUN_DATE`.
8. Commit and push state: `git add data/events.json data/watch_state.json data/runs/ data/candidates_$RUN_DATE.json data/harvest_$RUN_DATE.json && git commit -m "im-open-houses: scan $RUN_DATE (<N> new)"` then `git push origin HEAD:main || git push origin HEAD:claude/im-open-houses-state`. Never force-push, never touch files outside `im-open-houses/data/`.
